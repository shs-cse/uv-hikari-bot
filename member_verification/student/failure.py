...
async def check_if_matches_advising_server(member: hikari.Member, student_id: int) -> None:
    ...
    if str(member.id) in state.students[ADVISING_DISCORD_ID_COL].values:
        conflict_id = state.students[ADVISING_DISCORD_ID_COL] == str(member.id)
        conflict_id = state.students.index[conflict_id][0]
        if conflict_id == student_id:
            return  # same person
    ...
    # member probably has alt account -> sure?
        ...
        raise VerificationFailure(
            Response(
                comment, kind=Response.Kind.WAITING, components=YesNoButtonsView(member, student_id)
            )
        )
