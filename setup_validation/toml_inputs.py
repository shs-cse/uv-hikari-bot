...
from bot_environment.config import FilePath, InfoKey, RegexPattern
...
def has_info_passed_before() -> bool:
    ...
    if all(state.info[key] == passed[key] for key in state.info if key != InfoKey.MARKS_BUTTONS):
        ...
        return True
    # mismatch -> needs checking each toml key
    print(FormatText.warning("Needs checking every toml input key..."))
    # remove valid file
...
# check if info file contains all the toml keys
def check_info_keys() -> None:
    for attr, key in vars(InfoKey).items():
...
def check_regex_patterns() -> None:
    keys_and_regex_patterns = {
        InfoKey.COURSE_CODE: RegexPattern.COURSE_CODE,
        InfoKey.COURSE_NAME: RegexPattern.COURSE_NAME,
        InfoKey.SEMESTER: RegexPattern.SEMESTER,
        InfoKey.ROUTINE_SHEET_ID: RegexPattern.GOOGLE_DRIVE_LINK_ID,
        InfoKey.ST_SHEET_ID: RegexPattern.ST_SHEET_ID,
        InfoKey.MARKS_FOLDER_ID: RegexPattern.GOOGLE_DRIVE_LINK_ID,
        InfoKey.GUILD_ID: RegexPattern.DISCORD_ID,
        InfoKey.BOT_TOKEN: RegexPattern.DISCORD_BOT_TOKEN,
    }
    # check each of the toml keys in a loop
...
def check_and_load_sections() -> None:
    last_sec = state.info[InfoKey.LAST_SECTION]
    missing_secs = state.info[InfoKey.MISSING_SECTIONS]
    # make sure positive
    if last_sec <= 0:
...
def load_sections() -> None:
    ...
    state.available_secs = [
        sec
        for sec in range(1, state.info[InfoKey.LAST_SECTION] + 1)
        if sec not in state.info[InfoKey.MISSING_SECTIONS]
    ]
